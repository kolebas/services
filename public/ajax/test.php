...
<?
#Вывод списка групп из OU Active Directory	
...
$photo = CFile::GetPath($rs["PERSONAL_PHOTO"]);
$photoNew = $rs["LAST_NAME"].".png";
echo $rs["LAST_NAME"] ." ". $rs["NAME"];

if (!copy($_SERVER['DOCUMENT_ROOT'].$photo, $photoNew)) {
	$errors= error_get_last();
	echo "<br>COPY ERROR: ".$errors['type'];
    echo "<br />\n".$errors['message'];
    echo "<br>не удалось скопировать $photo...\n";
}
*/
...
function rus2translit($string) {
	...
	function checkOrgList($org){
		//Получаем путь OU в Active Directory для организации
		$id = 67;
		$arSelect = [
			NAME,
			'PROPERTY_AD_OU_USER',
		...
		while($arElements = $element->Fetch())
		{
			...
			if($arrElems[$i][NAME] != NULL)
				{
					$orgAdOu = $arrElems[$i][PROPERTY_AD_OU_USER_VALUE];
                    return $orgAdOu;
				}	
			$i++;
		}
	...
	ldap_set_option($ldap_con, LDAP_OPT_PROTOCOL_VERSION, 3);
$company = 'АО «ПИР ПРОДУКТ»';
$orgAdOu = checkOrgList($company);
	if(isset($orgAdOu)){
		$ou=$orgAdOu;
		echo $ou;
	}
	...
	if(ldap_bind($ldap_con, $ldap_dn, $ldap_password)) {
		...
		$ldaprecord['mobile'] = "{{Контактный телефон}}";
                $ldaprecord['company'] = $company;
		$ldaprecord['title'] = "{{Должность}}";
		...
		if(!($r = ldap_add($ldap_con,$dn,$ldaprecord))){
			$this->SetVariable('Status_auto_user_create', 'Неудача - ' . ldap_error($ldap_con) . ' ---- ' . $dn . ' ---- ' . $orgAdOu . ' ---- ' . $company);
		}
...
<?
# Изменение пользователя в Active Directory
/*	$usr = '{{Получатель услуги (имя)}}';
	preg_match('/\((.+)\)/', $usr, $usr_m);
	...
	$data = ldap_get_entries($ldap_con, $sr_usr);
        for ($i=0; $i<$data["count"]; $i++) {
			$dn_usr = $data[$i]["dn"];
			$entry["title"] = array("example@example.com");		
			echo $dn_usr;
			if(!($results = ldap_mod_replace($ldap_con,$dn_usr,$entry))){
				echo $results."<br>";
				echo 'Ошибка при изменении пользователя';
				}
			else {
