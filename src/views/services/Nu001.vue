        ...
        <v-row>
            <v-card
                ...
                <v-row class="mb-n6 mt-n8">
                    ...
                    <v-col cols="4">
                        <v-switch v-model='out_user' @change="chngSwitch()" inset class="mt-1" label='Доступ для внешнего сотрудника'></v-switch>                
                    </v-col>
                </v-row>
                ...
                <v-expand-transition>
                <Select 
                    v-if='org_name == "АО Агрохолдинг «СТЕПЬ»" || org_name == "ООО «Торговый Дом Агрохолдинг «СТЕПЬ»" || org_name == "ООО «Техюгснаб»"'
                    title='Департамент:'
                ...
                <SelectUsr :cols_title='4' :cols_input='6' title='ФИО руководителя:' :userId_err='mng_err'/>
                <v-row class='mb-n6'>
                    <v-col cols='4'>
                        <v-card-text class='subtitle-1 text-right pt-2'>
                            Контактный телефон:
...
export default {
    ...
    data:() => ({
        ...
        arr: [],        
        inputs: [
            {id: '1', title: 'ФИО:', err: '', label: 'Например: Крючков Иван Петрович', value: null, icon:'mdi-account', cols_title: '4', cols_input: '6'},
            {id: '2', title: 'Должность:', err: '', label: 'Например: Весовщик', value: null, icon:'mdi-briefcase-account', cols_title: '4', cols_input: '6'}
        ],
        dep: '',
        dep_err: '',        
        org_name: '',
    ...
    methods: {
        ...
        chngSwitch(){
            if(this.out_user){
                this.route = '1';
                this.dialog = true;
                this.dialogMessage = 'Сотруднику будет предоставлен удаленный доступ, при этом заявка будет отправлена на дополнительное согласование';
                
            }
        },
        formSend(){
            if (this.inputs[0].value && this.inputs[1].value && this.org_name && this.tel && this.date_start && this.date_burn){
                this.obj.fio = this.inputs[0].value;
                this.obj.wp = this.inputs[1].value;
                this.obj.out_user_mail = this.out_user_mail;
            ...
            if(!this.tel){
                this.tel_err = 'Необходимо указать контактный телефон нового сотрудника'
            }
            if(!this.date_start){
