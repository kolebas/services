<template>
  <v-container fluid>
    <div class="text-center">
      ...
      <v-dialog v-model="softModal" width="500" persistent>
        <v-card>
          <v-card-title
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="primary" text @click="closeSoftModal()">
              Понятно
            </v-btn>
          </v-card-actions>
        </v-card>
...
export default {
  ...
  data: () => ({
    ...
    sub_message:
      "В рамках исполнения данной заявки вам будет предоставлен доступ к программному обеспечению из предложенного списка (Для закупки программного обеспечения вам необходимо воспользоваться заявкой за закупку). Статус созданной заявки вы можете отслеживать в разделе ",
    dialogMessage: "",
    ...
    softModal: "",
    softModalMsg:
      "В связи с ограничением лицензионного соглашения, данное программное обеспечение предоставляется только сотрудникам офиса в г. Ростове-на-Дону",
    cmnt: "",
    userId_err: "",
    type_err: "",
    btnLoader: false,
